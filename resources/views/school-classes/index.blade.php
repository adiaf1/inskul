...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card">
        <div class="card-body">
                    ...
                    <tbody>
                        @forelse($schoolClasses as $class)
                            <tr>
                                ...
                                <td class="text-end">
                                    <button type="button" class="btn btn-sm btn-icon btn-label-primary" data-bs-toggle="offcanvas" data-bs-target="#offcanvasEditClass{{ $class->id }}" title="Edit" aria-label="Edit">
                                        <i class="bx bx-edit-alt"></i>
                                    </button>
                                    <form method="POST" action="{{ route('school-classes.destroy', $class) }}" class="d-inline" id="delete-class-{{ $class->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-sm btn-icon btn-outline-danger" onclick="confirmDeleteClass(@js($class->id))" title="Hapus" aria-label="Hapus">
                                    ...
                                    <form method="POST" action="{{ route('school-classes.update', $class) }}">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <select class="form-select" id="edit_academic_year_id_{{ $class->id }}" name="academic_year_id" required>
                                                @foreach($academicYears as $academicYear)
                                                    <option value="{{ $academicYear->id }}" @selected((string) old('academic_year_id', $class->academic_year_id) === (string) $academicYear->id)>
                                                        {{ $academicYear->name }}
                                                    </option>
                                                @endforeach
                                            </select>
            ...
            <x-table-pagination :paginator="$schoolClasses" label="kelas" />
        </div>
    </div>
