...
<div class="mb-4">
    ...
    <select class="form-select" id="{{ $mode }}_student_ids" name="student_ids[]" multiple size="8">
        @foreach($students as $student)
            <option value="{{ $student->id }}" @selected(in_array((string) $student->id, array_map('strval', $selectedStudents), true))>
                {{ $student->user?->name }}{{ $student->nis ? ' - NIS '.$student->nis : '' }}
            </option>
        @endforeach
    </select>
